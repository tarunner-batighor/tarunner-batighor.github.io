...
export async function renderLeaderboard(container) {
  container.innerHTML =
    '<div class="lb-wrap page-anim">' +
      '<div style="text-align:center;margin-bottom:8px"><h1 class="page-title">🏆 সেরা লেখক তালিকা</h1>' +
      ...
      '<div id="lbLoading" class="loading-block"><div class="loader"></div><div id="lbProgress">📊 ডেটা গণনা হচ্ছে…</div></div>' +
  ...
  try {
    ...
    if (!postDocs.length) {
      container.querySelector("#lbLoading").outerHTML =
        '<div class="empty-state"><div class="es-icon">🪶</div><h3>এখনো কোনো প্রকাশিত লেখা নেই</h3></div>';
      return;
    }
    ...
    await poolMap(postDocs, 6, async function (sd) {
      ...
      if (progress && done % 5 === 0) progress.textContent = "📊 গণনা চলছে… " + bn(done) + "/" + bn(postDocs.length);
    });
    ...
    container.querySelector("#lbPodium").innerHTML =
      '<div class="lb-podium">' + top3.map(function (e, i) {
        return '<div class="podium-card ' + CLS[i] + '" data-uid="' + escapeHtml(e.uid) + '">' +
          ...
          '<div class="podium-sub">' + bn(e.posts) + " লেখা · 👁️ " + bn(e.views) + " · ❤️ " + bn(e.likes) + "</div>" +
        "</div>";
      }).join("") + "</div>";
    ...
    container.querySelector("#lbList").innerHTML = entries.slice(3).map(function (e, i) {
      ...
      return '<div class="lb-row" data-uid="' + escapeHtml(e.uid) + '">' +
        ...
        '<div class="sub">📚 ' + bn(e.posts) + " লেখা · 👁️ " + bn(e.views) + " · ❤️ " + bn(e.likes) + " · 💬 " + bn(e.comments) + "</div></div>" +
      ...
      '<div class="best-posts" id="best-' + escapeHtml(e.uid.replace(/[^a-zA-Z0-9_-]/g, "")) + '">' +
        e.best.map(function (p) {
          return '<a href="#/post/' + p.id + '">📖 ' + escapeHtml(p.title) + ' <span class="vp">· 👁️ ' + bn(p.views) + "</span></a>";
        }).join("") +
      "</div>";
    }).join("");
    ...
    console.error(e);
    container.innerHTML = '<div class="empty-state"><div class="es-icon">📡</div><h3>লিডারবোর্ড লোড হয়নি</h3><p>' +
