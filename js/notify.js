...
function panelHtml() {
  if (!list.length) {
    return '<div class="sg-empty">🔕 কোনো নোটিফিকেশন নেই</div>';
  }
  return list.slice(0, 30).map(function (n) {
    const icon = n.type === "rejected" ? "❌" : (n.submission ? "📥" : "✅");
    return '<button type="button" class="sg-item nf-item' + (n.read ? "" : " unread") + '" data-id="' + n.id +
      ...
      '<span style="min-width:0;flex:1">' +
        '<span class="sg-title" style="display:block;white-space:normal;font-weight:' + (n.read ? "500" : "700") + '">' + escapeHtml(n.message || n.postTitle || "") + '</span>' +
